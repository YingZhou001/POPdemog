...
to the real population size at the particular time. We can ignore the
sizes difference by setting `size.scale="topology"` and use
`toposize.scale` to fit the circle sizes to the plot. We can also set
`size.scale="log"` and `size.scale="linear"` to scale the circle size
according to the population size. `"linear"` option will clearly give
different circle sizes according to the population sizes, while `"log"`
...
Displaying real population sizes
--------------------------------

*NRuler()* and *NOut()* are two functions to display real population
size. *NRuler()* adds a population size ruler to the plot, and user can
customize the ruler position, ticks on the ruler, labels on the ticks,
and other figure options. *NOut()* function outputs the numbers of the
real population sizes. It can be used with *legend()* function to
annotate the real population sizes to the plot.  
...
the plot generated by *PlotMS()* and *NOut()* is for the plot generated
by *PlotMig()*. We do not suggest to use *NRuler()* to the migration
plot because it is not easy check the population size from the circle
size while the ruler only tells about the radius length.
