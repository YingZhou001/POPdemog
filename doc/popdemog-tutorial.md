...
The package POPdemog produces plots which visualize population
...
deal with complex migration events.  *PlotMMig()* gives the overview of
migrations over time, and *PlotMig()* shows migrations at a specified
time point in fine detail. In addition, there is a function *NRuler()*
to add population size ruler to the plot.
...
of POPdemog to visualize population history. We also attached abundant
...
The `input.file` contains the all information for demographic history,
it can be a copy of the simulation command line or the parameter file.
...
We will store the preceding ms command line in the "sample.ms.cmd", and
use it in examples later in this tutorial.
...
has the same meaning of 4*N*0 in ms, which must be specified in order to
give the right times for demographic events and the accurate sizes for
populations.

The `N4` parameter is not used in two situations: 1) when producing a
topology plot with `size.scale="topology"`; 2) when the simulation
script do not re-scale the population size and time, such as is the case
with Cosi.
...
file "sample.ms.cmd", `N4` set to 10000 in this case.
...
with the source population by default, or can be reset with the
`col.arrow` parameter.
...
*l**o**g*<sub>10</sub>(*y**e**a**r* *b**e**f**o**r**e* *p**r**e**s**e**n**t*),
using default 25 years per generation. The year per generation can be
...
font size, axis label size, and the arrow size to fit to diverse plot
region.
...
When there are multiple migrations at the particular time, arrows in
*PlotMS()* might cross with the population lineages and overlap with
each other. The migration events will be difficult to see in this
setting, so we use function *PlotMig()* to output the migrations on the
specified time in separate plots. We also have *PlotMMig()* function to
display an overview of all possible migrations among simulated
populations. Both these two migration plot functions are based on the
...
plot all migrations at time 125 years ago (with `time_pt=125` and
`time.scale="year"`). The time point(`time_pt`) is on the same scale as
the setting of `time.scale`.
...
to ms-compatible command, and then plot them out. Here we give an
...
Then we can use functions *PlotMS()* and *PlotMig()* to plot the
demographic history.
...
This POPdemog package can also support the scrm simulator[10]. Here we
make an example simulation script that changes
